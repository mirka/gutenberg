...
async function runPerformanceTests( branches, options ) {
	...
	await runShellScript(
		`bash -c "source $HOME/.nvm/nvm.sh && nvm install && npm install --global npm@11 && npm ci && npm run build --workspace @wordpress/e2e-test-utils-playwright && npx playwright install chromium --with-deps"`,
		testRunnerDir
	);
	...
	for ( const branch of branches ) {
		...
		await runShellScript(
			`bash -c "source $HOME/.nvm/nvm.sh && nvm install && npm install --global npm@11 && npm ci && (npm run build -- --skip-types || npm run build)"`,
			buildDir
		);
